...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print()
    # exact=True throughout the breakdown: a reader should be able to check that
    # these lines subtract to the one below them, which rounded rupees would break.
    print(f"  gross captured   {fmt_inr(m['gross_captured'], exact=True):>16}")
    print(f"  - fees           {fmt_inr(m['fees'], exact=True):>16}")
    print(f"  - gst            {fmt_inr(m['gst'], exact=True):>16}")
    print(f"  {'':<16} {'-' * 16}")
    print(f"  = reaches bank   {fmt_inr(m['net_settled'], exact=True):>16}")
    print(f"  - refunds        {fmt_inr(m['refunds'], exact=True):>16}")
    print(f"  - chargebacks    {fmt_inr(m['chargebacks'], exact=True):>16}")
    print(f"  - outflows       {fmt_inr(m['outflows_in_window'], exact=True):>16}")
    print()
